...
namespace SphericalHarmonicSolver
{
    class Program
    {
        private static object EnvMapLock;
        private static double[][] OutputData;
        private static object[] OutputLocks;
        private static int[] WriteCount;

        private static Color Sample(Vector3 norm)
        {
            norm = norm / norm.Length();

            int x = (int)(norm.X * EnvMap.Width / 2 + EnvMap.Width);
            int y = (int)(norm.Y * EnvMap.Height);

            return EnvMap.GetPixel(x, y);
        }

        private static double Sample(double theta, double phi, int col)
        {
            lock (EnvMapLock)
            {
        ...
        private static void Store(int x, int y, double theta, double phi, double r_d, double g_d, double b_d)
        {
            /*int x = 0, y = 0, w = 0;
            lock (EnvMapLock)
            {
        ...
        private static double Factorial(int v)
        {
            if (v <= 1)
                return 1;
        ...
        private static double DFactorial(int v)
        {
            if (v <= 1)
                return 1;
        ...
        private static double P(int l, int m, double theta)
        {
            if (l == 0 && m == 0)
                return 1;
            else if (l == 1)
        ...
        private static double P_recur(int l, int m, double x)
        {
            if (l == 0 && m == 0)
                return 1;
        ...
        static double MonteCarlo(int l, int m, int col, int phi_sample_cnt, int theta_sample_cnt)
        {
        ...
        static void Render(double[][] coeff, int l, int off)
        {
            int w = 0, h = 0;
            lock (EnvMapLock)
            {
